...
(function($) {
    ...
    $.validationEngine = {
        ...
        validateCall : function(caller,rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
            ...
            for (var i=0; i<rules.length;i++){
                switch (rules[i]){
                    ...
                    case "funcCall":
                        ...
                        break;
                    case "parsianFuncCall":
                        _parsianFuncCall(caller,rules,i);
                        break;
                    default :
                }
                if(promptText != "")break;
            ...
            function _parsianFuncCall(caller,rules,position){
                var customRule = rules[position+1];
                var funce = $.validationEngine.settings.allrules[customRule].nname;
            ...
            function _length(caller,rules,position){    	  // VALIDATE LENGTH
                ...
                if(feildLength<startLength || feildLength>endLength){
                    $.validationEngine.isError = true;
                    promptText += $.validationEngine.settings.allrules[rules[i]].alertText+startLength+$.validationEngine.settings.allrules[rules[i]].alertText2+endLength+$.validationEngine.settings.allrules[rules[i]].alertText3+"<br />";
                }
            }
        ...
        calculatePosition : function(caller,promptText,type,ajaxed,divFormError){
            ...
            if($.validationEngine.settings.promptPosition == "topLeft"){ callerTopPosition += -inputHeight; }
        ...
        submitValidation : function(caller) {					// FORM SUBMIT VALIDATION LOOPING INLINE VALIDATION
            ...
            $(caller).find("[class*=validate]").not(':hidden').each(function(){ // Bitarts
                if(!this.disabled){ // Bitarts
