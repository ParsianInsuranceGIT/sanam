...
$.extend($.jgrid,{
	...
	parseDate : function(format, date) {
		var tsp = {m : 1, d : 1, y : 1970, h : 0, i : 0, s : 0},k,hl,dM, regdate = /[\\\/:_;.,\t\s-]/;
				...
				if (ln && ln>0){
					while(i<ln){
						if($($t.rows[i]).hasClass('jqgrow')) {
							...
							if(mathopr) {
//Parsian
                                val2 = jQuery.global.parseInt(val)
								v = parseFloat(val2);
								sum += v;
								min = Math.min(min, v);
								max = Math.max(min, v);
...
$.jgrid.extend({
	...
	GridNav : function() {
		return this.each(function () {
			...
			$t.p.knv = $t.p.id + "_kn";
			var selection = $("<span style='width:0px;height:0px;background-color:black;' tabindex='0'><span tabindex='-1' style='width:0px;height:0px;background-color:gray;' id='"+$t.p.knv+"'></span></span>"),
			i, kdir;
